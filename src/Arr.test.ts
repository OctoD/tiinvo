...
describe("Arr", () =>
{
  ...
  test(Arr.getOr.name, () =>
  {
    // Returns the element `Option.t<a>` at index `i` of an array `T<A>` or `A` if the index is out of range.
    expect(Arr.getOr([10, 20], 5, 0)).toEqual(10);
    expect(Arr.getOr([10, 20], 5, 1)).toEqual(20);
    expect(Arr.getOr([10, 20], 5, 22)).toEqual(5);

    // Returns a `Fn.Unary<t<a>, A>` to get the element `Option.t<a>` at index `i` of an array `T<A>` or `A` if the index is out of range.
    expect(Arr.getOr(5)([10, 20], 0)).toEqual(10);
    expect(Arr.getOr(5)([10, 20], 1)).toEqual(20);
    expect(Arr.getOr(5)([10, 20], 20)).toEqual(5);
  });
